...
std::tuple<glm::vec3, glm::vec3> StaticMesh::getAABB() {
	if(AABBInitialized)
	{
		return AABBBounds;
	}
    ...
    std::cout << "StaticMesh AABB calculated - Min: ("
        << minBounds.x << ", " << minBounds.y << ", " << minBounds.z << ")" << std::endl;
    std::cout << "StaticMesh AABB calculated - Max: ("
        << maxBounds.x << ", " << maxBounds.y << ", " << maxBounds.z << ")" << std::endl;


	AABBBounds = {minBounds, maxBounds};
